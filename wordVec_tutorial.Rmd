...
jazziness_and_countryness = jazziness_and_countryness[
  rank(-jazziness_and_countryness[,1])<20 |
  rank(-jazziness_and_countryness[,2])<20,
  ]
...
We can also plot these gendered words to get a sense of how words are represented in space. If you imagine an arrow connecting the gendered terms, you can see that they go in one direction. Man --> Woman; Joe --> Godiva; Guy --> Lady. The male term --> female term.
...
top_evaluative_words %>%
  inner_join(goodness) %>%
  inner_join(masculinity) %>%
  ggplot() + 
  geom_text(aes(x=`similarity to "him" - "her"`,
                y=`similarity to "best" - "worst"`,
                label=word)) +
  geom_abline(a=0,b=1)
```
